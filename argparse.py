...
# 单分派，在python实现泛型方法，post get一样参数
@singledispatch
...
# def _info_compara_methods(class_=None,compara=None,format='json'):
#     response =requests.get(f"https://rest.ensembl.org/info/compara/methods", headers={"Content-Type": "application/json"}, 
#                                params=dict(class=class_,compara=compara,format=format))
#     return response.json()
...
#     response =requests.get(f"https://rest.ensembl.org/info/variation/populations/{population_name}", headers={"Content-Type": "application/json"}, 
...
def _ga4gh_variantannotations_search(variantAnnotationSetId=None,effects=None,end=None,pageSize=None,pageToken=None,referenceId=None,referenceName=None,start=None,format='json'):
    response =requests.post(f"https://rest.ensembl.org/ga4gh/variantannotations/search", headers={"Content-Type": "application/json"}, 
                            json=dict(variantAnnotationSetId=variantAnnotationSetId,effects=effects,end=end,pageSize=pageSize,pageToken=pageToken,referenceId=referenceId,referenceName=referenceName,start=start),params=dict(format=format))
    return response.json()
...
info_compara_methods.add_argument('--class',default=None)
...
vep_region_post.add_argument('variants')
...
if args.command == "variant_recoder":
...
elif args.command == "nfo_compara_methods":
    pprint.pprint(_info_compara_methods(_class=args._class,compara=args.compara,format=args.format))
...
elif args.command == "info_genomes_name":
    pprint.pprint(_info_genomes_name(args.taxon_name,expand=args.expand,format=args.format))
    ...
    pprint.pprint(_phenotype_gene(args.gene,species=args.species,include_children=args.include_children,include_pubmed_id=args.include_pubmed_id,include_review_status=args.include_review_status,source=args.source,format=args.format))
